...
const input={...parseIdentity('KONE SALI 456283902 CAT2 33ans 08354290926'),sex:'F',start:'2026-09-20',end:'2026-09-23',registrar:'KOUAME AFFOUE SABINE',cashier:'AKA OI AKA LAURENT'};
test('Exemple demandé : 3 jours CAT2, 60 000 FCFA, zéro initial intact',()=>{const d=receiptData(input,new Date('2026-09-24T00:00:20Z'));assert.equal(d.receipt,'08354290926');assert.equal(d.isn,'456283902');assert.equal(d.total,60000);assert.equal(d.days,3);assert.equal(d.unit,20000);assert.deepEqual(d.paid,{date:'23/09/2026',time:'23:59:40',short:'23/09/2026 23:59'});});
...
test('Noms composés, espaces, casse et âge sans suffixe',()=>{const d=parseIdentity('  N’GUESSAN ANNE-MARIE 00045 cat 3 70 0012 ');assert.equal(d.name,'N’GUESSAN ANNE-MARIE');assert.equal(d.isn,'00045');assert.equal(d.receipt,'0012');assert.equal(d.age,70);});
